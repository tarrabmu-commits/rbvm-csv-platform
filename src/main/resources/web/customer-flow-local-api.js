(() => {
  ...
  function statusNode(panel) {
    return panel.querySelector('.status-message');
  }
  ...
  function buttonByLabel(panel, label) {
  ...
  function readAssets(panel) {
    ...
    return editors.map((editor, index) => {
      ...
      const cr = readRequiredField(editor, 'CR', index);
      const ir = readRequiredField(editor, 'IR', index);
      const ar = readRequiredField(editor, 'AR', index);

      if (!customerAssetKey && !displayName) {
        throw new Error(`Asset ${index + 1} needs an Asset ID or Asset Name.`);
      }
      if (!CRITICALITY.includes(assetCriticality)) {
        throw new Error(`Asset ${index + 1} has invalid Asset Criticality.`);
      }
      if (!INTERNET_FACING.includes(internetFacing)) {
        throw new Error(`Asset ${index + 1} has invalid Internet Facing state.`);
      }
      if (!PUBLICLY_EXPOSED.includes(publiclyExposed)) {
        throw new Error(`Asset ${index + 1} has invalid CISA Publicly Exposed state.`);
      }
      if (![cr, ir, ar].every(value => SECURITY_REQUIREMENT.includes(value))) {
        throw new Error(`Asset ${index + 1} has invalid CVSS v4 CR/IR/AR values.`);
      }
      if (assetCriticality === 'UNKNOWN' || internetFacing === 'UNKNOWN') {
        throw new Error(`Asset ${index + 1} still needs Asset Criticality and Internet Facing for the existing customer-context workflow. Publicly Exposed may remain UNKNOWN; CR/IR/AR may remain X.`);
      }
  ...
  function waitFor(predicate, timeoutMs = 3000) {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        observer.disconnect();
        reject(new Error('Saved customer data could not be rendered in the asset editor.'));
      }, timeoutMs);
  ...
  async function hydrateSavedBundle(panel, runId) {
    ...
    try {
      ...
      if (bundle.contractId !== BUNDLE_CONTRACT || bundle.schemaVersion !== 4 || !Array.isArray(bundle.assets)) {
        throw new Error(`Saved run data is not ${BUNDLE_CONTRACT} schema version 4.`);
      }

      const upload = panel.querySelector('input[type="file"][accept*="json"]');
      if (!upload || typeof DataTransfer === 'undefined' || typeof File === 'undefined') {
        throw new Error('This browser cannot restore saved customer data into the editor.');
      }
      ...
      setStatus(panel, `Restored saved customer data for ${bundle.assets.length} asset${bundle.assets.length === 1 ? '' : 's'} from this local run.`, 'success');
    } catch (error) {
      setPersisted(panel, false);
  ...
  async function persist(panel, runId, saveButton) {
    if (!runId) {
      setStatus(panel, 'Open Assets from a CSV-first run before saving customer data locally.', 'error');
      return;
    }
    ...
    try {
      ...
      await request(`${API_ROOT}/${encodeURIComponent(runId)}`, {
        method: 'PUT',
      ...
      const suffix = bodIncomplete
        ? ` ${bodIncomplete} asset${bodIncomplete === 1 ? '' : 's'} remain BOD-incomplete because Publicly Exposed is UNKNOWN.`
        : '';
  ...
  async function downloadSaved(panel, runId, downloadButton) {
    if (!runId) {
      setStatus(panel, 'No CSV-first runId is available for local customer data.', 'error');
      return;
    }
    ...
    try {
      ...
      link.href = href;
      link.download = 'rbvm-customer-assets-v4.json';
      ...
      downloadButton.disabled = panel.dataset.customerBundlePersisted !== 'true';
    }
  }
  ...
  function showAnalysisResult(panel, analysis, priority) {
    ...
    if (!host) {
      ...
      host.dataset.localAnalysisResult = 'true';
      host.className = 'stack';
      const status = statusNode(panel);
      if (status) status.insertAdjacentElement('afterend', host);
      else panel.querySelector('.panel-body')?.append(host);
    }
    host.replaceChildren();

    const summary = document.createElement('div');
    summary.className = 'callout callout-info';
  ...
  async function analyzeSaved(panel, runId, analyzeButton) {
    if (!runId || panel.dataset.customerBundlePersisted !== 'true') {
      setStatus(panel, 'Save customer data to this local run before analyzing it.', 'error');
      return;
    }
    ...
    try {
      ...
      analyzeButton.disabled = panel.dataset.customerBundlePersisted !== 'true';
    }
  }
  ...
  function enhance(panel) {
    ...
    analyzeButton.className = 'button button-secondary';
    analyzeButton.textContent = 'Analyze saved data';
    analyzeButton.disabled = true;
    saveButton.insertAdjacentElement('afterend', analyzeButton);

    if (runId) {
      saveButton.title = 'Persist this exact V4 customer bundle beside the CSV-first run.';
    ...
    panel.addEventListener('click', event => {
      ...
      if (label === 'Save customer data') {
        ...
        void persist(panel, runId, target);
      } else if (label === 'Download customer data') {
        event.preventDefault();
        event.stopImmediatePropagation();
        void downloadSaved(panel, runId, target);
      } else if (label === 'Analyze saved data') {
        event.preventDefault();
        event.stopImmediatePropagation();
        void analyzeSaved(panel, runId, target);
      }
    }, true);
  ...
  function scan() {
    document.querySelectorAll('[data-customer-asset-setup]').forEach(enhance);
  }
