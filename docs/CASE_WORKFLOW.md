...
| `SOURCE_RESOLVED` | كل Exposures حُلّت بدليل V2 صريح | لا |
...
لا يوجد انتقال آلي بسبب غياب CVE من CSV لاحقة. V1 إيجابي فقط؛ أما V2 فيسمح
بـ`SOURCE_RESOLVED` من صف `RESOLVED` صريح ومطابق، ويعيد `ACTIVE` الأحدث الحالة
إلى `OPEN`. قرارات Workflow البشرية لا يستبدلها الاستيراد.
