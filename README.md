...
Frontend V2 deliberately does **not** fabricate historical trends when the current API cannot support defensible historical aggregation. It now exposes exact replay-verified RBVM Formula V1 and derived-risk methodology results through explicit immutable identities, while remediation priority, SLA, treatment, and remediation ranking remain outside the browser risk-result surface.
...
The Formula Result runtime requires PostgreSQL schema V23+, is Viewer-protected before capability lookup, and deliberately has no `latest` selector. The production `FORMULA_RESULT_MATERIALIZATION_V1` path takes only one exact already-persisted Decision Input V3 SHA, resolves only that snapshot's captured evidence/bindings, evaluates Formula V1, append/replay installs the canonical explanation, reloads it, and requires historical replay before returning. The separate Operator-only `RBVM_FORMULA_RESULT_MATERIALIZATION_API_V1` exposes exactly that operation at `POST /api/v1/formula-result-materializations/{inputSnapshotSha256}`; it accepts no body or query selector. Frontend System V2 now presents exact replay-verified Formula V1 results through explicit Finding, Decision Input V3, and Formula catalog identities; the browser performs no Formula arithmetic or implicit current/latest selection.

See [`docs/DECISION_INPUT_V3.md`](docs/DECISION_INPUT_V3.md) for the immutable Decision Input boundary, [`docs/RBVM_FORMULA_V1.md`](docs/RBVM_FORMULA_V1.md) for the accepted Formula contract, [`docs/RBVM_FORMULA_CANONICALIZATION_V1.md`](docs/RBVM_FORMULA_CANONICALIZATION_V1.md) for canonical Formula/explanation identity, [`docs/FORMULA_RESULT_API_V1.md`](docs/FORMULA_RESULT_API_V1.md) for exact replay-verified Formula Result reads, and [`docs/FORMULA_RESULT_MATERIALIZATION_V1.md`](docs/FORMULA_RESULT_MATERIALIZATION_V1.md) for exact production materialization and Operator transport semantics.
...
- Operator-protected exact-snapshot Formula materialization HTTP command with no body/query selectors and authentication/authorization before runtime capability disclosure.
