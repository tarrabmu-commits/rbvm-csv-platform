#!/usr/bin/env python3
"""Enrich one uploaded CSV from public CVE intelligence without using tenant/database state.
...
that file, and normalized evidence is merged back by CVE_ID.
"""
...
def enrichment_columns(record, snapshot):
    ...
    result.update({
        ...
        "EPSS_Score_Date": text(epss.get("scoreDate")),
        "KEV_Listed": text(kev.get("listed")),
        "KEV_Date_Added": text(kev.get("dateAdded")),
        "KEV_Due_Date": text(kev.get("dueDate")),
        "KEV_Vendor_Project": text(kev.get("vendorProject")),
        "KEV_Product": text(kev.get("product")),
        "KEV_Vulnerability_Name": text(kev.get("vulnerabilityName")),
        "KEV_Known_Ransomware_Campaign_Use": text(kev.get("knownRansomwareCampaignUse")),
        "KEV_Required_Action": text(kev.get("requiredAction")),
        "KEV_Notes": text(kev.get("notes")),
        "CISA_Exploitation": text(ssvc.get("exploitation")),
        "CISA_Automatable": text(ssvc.get("automatable")),
        "CISA_Technical_Impact": text(ssvc.get("technicalImpact")),
        "CISA_SSVC_Version": text(ssvc.get("version")),
        "CISA_SSVC_Timestamp": text(ssvc.get("timestamp")),
        "NVD_Published": text(nvd.get("published")),
        "NVD_Last_Modified": text(nvd.get("lastModified")),
        "NVD_Status": text(nvd.get("vulnStatus")),
        "NVD_Source_Identifier": text(nvd.get("sourceIdentifier")),
        "NVD_Description": text(descriptions[0]) if descriptions else "",
        "CWE_JSON": json_cell(nvd.get("weaknesses") or []),
        "CPE_Criteria_JSON": json_cell(nvd.get("cpeCriteria") or []),
        "NVD_References_JSON": json_cell(nvd.get("references") or []),
        "CVE_State": text(cve_metadata.get("state")),
        "CVE_Assigner": text(cve_metadata.get("assignerShortName") or cve_metadata.get("assignerOrgId")),
        "CNA_Provider": text(cna.get("providerShortName") or cna.get("providerOrgId")),
        "CNA_Title": text(cna.get("title")),
        "Intel_Observed_At": text(snapshot.get("observedAt")),
        "Public_Intel_Snapshot_SHA256": text(snapshot.get("snapshotSha256")),
...
def main():
    ...
    output_headers = headers + ENRICHMENT_HEADERS
    write_csv(args.output, output_headers, enriched_rows)
    ...
    report = {
        ...
        "publicIntelSnapshot": str(snapshot_path),
        "publicIntelSnapshotSha256": snapshot.get("snapshotSha256"),
        "observedAt": snapshot.get("observedAt"),
        "cvssV4RowStatus": cvss_status,
        "databaseStateUsed": False,
        "scope": "INPUT_CSV_ONLY",
    }
