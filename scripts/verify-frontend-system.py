...
for needle in ("RBVM Frontend System V2","English-only","Overview","Findings","Assets","Analytics","Reports","Evidence","Imports","Settings","no in-app login","Decision Input Snapshot","Formula Contract"):
    if needle.lower() not in readme.lower():
        raise AssertionError(f"README is stale relative to Frontend V2: missing {needle!r}")
for forbidden in ("RBVM_FRONTEND_SYSTEM_V1","واجهة عربية","responsive RTL"):
    if forbidden.lower() in readme.lower():
        raise AssertionError(f"README contains stale Frontend V1 statement {forbidden!r}")
