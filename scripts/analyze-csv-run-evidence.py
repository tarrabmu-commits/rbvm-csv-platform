...
This analyzer deliberately keeps CVSS severity, EPSS probability, KEV/SSVC threat
signals, and customer context separate. It resolves only the CVSS v4 Threat E
metric when published evidence is sufficient. Asset-level Internet Facing is not
silently converted to MAV, and scalar Asset Criticality is not converted to
CR/IR/AR.
"""
...
ANALYSIS_COLUMNS = [
    ...
    "Asset_Criticality",
    "Internet_Facing",
    "CVSS4_Threat_E_Status",
    "CVSS4_Threat_E_Resolved",
    "CVSS4_CR_Resolved",
    "CVSS4_IR_Resolved",
    "CVSS4_AR_Resolved",
    "CVSS4_MAV_Resolved",
...
def analyze_row(row, asset, context_status):
    ...
    # Environmental metrics intentionally remain Not Defined in the MVP.
    cr = ir = ar = mav = "X"
    if row.get("CVSS4_Status") != "PRESENT":
        mode = "UNAVAILABLE"
    ...
    if row.get("CVSS4_Status") != "PRESENT":
        blockers.append("CVSS4_BASE_NOT_PRESENT")
    if not str(row.get("EPSS_Probability") or "").strip():
        blockers.append("EPSS_MISSING")
    if context_status not in {"MATCHED_KEY", "MATCHED_NAME"}:
        blockers.append("CUSTOMER_CONTEXT_NOT_MATCHED")
    if criticality == "UNKNOWN":
        blockers.append("ASSET_CRITICALITY_UNKNOWN")
    if internet == "UNKNOWN":
        blockers.append("INTERNET_FACING_UNKNOWN")
    blockers.extend([
        "CR_IR_AR_NOT_DEFINED",
        "INTERNET_FACING_NOT_EQUIVALENT_TO_MAV",
    ])
...
def main():
    ...
    summary = {
        ...
        "benchmarkFields": [
            "Severity", "CVSS4_Base_Score", "CVSS4_Base_Severity", "EPSS_Probability",
            "KEV_Listed", "CISA_Exploitation", "CISA_Automatable", "CISA_Technical_Impact",
            "Asset_Criticality", "Internet_Facing",
        ],
        "rbvmV2": {
